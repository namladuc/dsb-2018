...
def get_dsb2018_fingerprint(data_root, max_samples=None, verbose=True):
    ...
    # Get all image IDs
    image_ids = [d for d in os.listdir(train_path) 
    ...
    if verbose:
        ...
        print(f"\nImage sizes:")
        print(f"  Height: {fingerprint['image_sizes']['height']['min']} - {fingerprint['image_sizes']['height']['max']} "
              f"(mean: {fingerprint['image_sizes']['height']['mean']:.1f})")
        print(f"  Width:  {fingerprint['image_sizes']['width']['min']} - {fingerprint['image_sizes']['width']['max']} "
        ...
        print(f"Mean intensity: {fingerprint['intensity']['mean']['mean']:.2f} ± {fingerprint['intensity']['mean']['std']:.2f}")
...
def generate_preprocessing_params(fingerprint):
    ...
    params = {
        ...
        'original_size_range': {
            'height': [fingerprint['image_sizes']['height']['min'], 
                      fingerprint['image_sizes']['height']['max']],
            'width': [fingerprint['image_sizes']['width']['min'], 
