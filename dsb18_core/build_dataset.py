...
def get_train_valid_dataset_dsb2018(CFG, path_data):
    ...
    if normalization_method == 'zscore':
        print(f"  Global normalization - Mean: {fingerprint['intensity']['mean']['mean']:.2f}, "
              f"Std: {fingerprint['intensity']['mean']['std']:.2f}")
    elif normalization_method == 'minmax':
        print(f"  Min-Max normalization - Min: {fingerprint['intensity']['global_min']:.0f}, "
    ...
    print(f"Image size range: {df['height'].min()}x{df['width'].min()} to {df['height'].max()}x{df['width'].max()}")
    print(f"Nuclei count: {df['num_nuclei'].min()} - {df['num_nuclei'].max()} (mean: {df['num_nuclei'].mean():.1f})")
