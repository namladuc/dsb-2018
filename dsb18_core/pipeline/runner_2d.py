...
def run_training2d(model, optimizer, scheduler, run, num_epochs, train_loader, valid_loader, CFG):
    ...
    for epoch in range(CFG.epochs_res, num_epochs + 1):
        ...
        if current_metric > best_metric_value:
            print(f"✓ {primary_metric.upper()} improved "
            ...
            best_model_wts = copy.deepcopy(model.state_dict())
            best_path = os.path.join(exp_dir, f"best_epoch_{CFG.model_name}_{CFG.fold_selected:02d}.bin")
            torch.save(model.state_dict(), best_path)
            if CFG.using_wandb:
                wandb.save(best_path, base_path=exp_dir)
        
        # Save last checkpoint
        last_path = os.path.join(exp_dir, f"last_epoch_{CFG.model_name}_{CFG.fold_selected:02d}.bin")
