# https://github.com/givkashi/Awesome-unet-like-transformers

# https://github.com/McGregorWwww/UCTransNet/tree/main
# UCTransNet
...
class Channel_Embeddings(nn.Module):
    ...
    def __init__(self, config, patchsize, img_size, in_channels):
        ...
        self.dropout = Dropout(config.emb_drop_rate)
...
class Attention_org(nn.Module):
    def __init__(self, config, vis, channel_num):
        ...
        self.attn_dropout = Dropout(config.att_drop_rate)
        self.proj_dropout = Dropout(config.att_drop_rate)
    ...
    def forward(self, emb1, emb2, emb3, emb4, emb_all):
        ...
        context_layer1 = (
            context_layer1.permute(0, 3, 2, 1).contiguous() if emb1 is not None else None
        )
        context_layer2 = (
            context_layer2.permute(0, 3, 2, 1).contiguous() if emb2 is not None else None
        )
        context_layer3 = (
            context_layer3.permute(0, 3, 2, 1).contiguous() if emb3 is not None else None
        )
        context_layer4 = (
            context_layer4.permute(0, 3, 2, 1).contiguous() if emb4 is not None else None
        )
...
class Mlp(nn.Module):
    def __init__(self, config, in_channel, mlp_channel):
        ...
        self.act_fn = nn.GELU()
        self.dropout = Dropout(config.drop_rate)
        self._init_weights()
...
class ChannelTransformer(nn.Module):
    def __init__(
        self, config, vis, img_size, channel_num=[64, 128, 256, 512], patchSize=[32, 16, 8, 4]
    ):
        super().__init__()
