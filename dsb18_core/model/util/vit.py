...
class DecoderTransformer(nn.Module):
    def __init__(
        ...
        patch_dim,
        CFG,
        decoder_channel=[48, 96, 192, 384, 768],
        decoder_factor=[1, 2, 4, 8, 16],
        ...
        self.conv_filter = nn.ModuleList(
            [
                nn.Conv2d(in_channels=decoder_channel[i], out_channels=num_classes, kernel_size=1)
                for i in range(len(decoder_channel))
            ]
        )

        self.upsample_size = nn.ModuleList(
            [nn.Upsample(scale_factor=decoder_factor[i]) for i in range(len(decoder_factor))]
        )

    def forward(self, features):
        upSize = [self.upsample_size[i](features[i]) for i in range(len(features))]

        reduceChannel = [self.conv_filter[i](upSize[i]) for i in range(len(upSize))]
...
if __name__ == "__main__":
    ...
    print(
        rearrange(
            x, "b c (patch_x x) (patch_y y) -> b (x y) (patch_x patch_y c)", patch_x=16, patch_y=16
        ).shape
    )
