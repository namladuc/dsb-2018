...
class Down2D(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.encoder = nn.Sequential(nn.MaxPool2d(2, 2), DoubleConv(in_channels, out_channels))
...
class Conv2dReLUFire(nn.Sequential):
    def __init__(
        ...
        conv = FireModule(in_channels, out_channels // 2, out_channels)
        relu = nn.ReLU(inplace=True)
...
class FireModuleV2(nn.Module):
    def __init__(self, in_channels, squeeze, expand, expand_ratio=2, expand_kernel=3):
        ...
        self.expand3x3 = nn.Conv2d(
            squeeze, expand // expand_ratio, kernel_size=expand_kernel, padding=expand_kernel // 2
        )
        self.expand3x3_activation = nn.ReLU(inplace=True)
...
class Down2FireModuleResAvg(nn.Module):
    def __init__(self, in_channels, out_channels, avg_conv_use=True):
        ...
        self.avg_conv = nn.Conv2d(out_channels, out_channels, kernel_size=2, stride=2)
...
class Up1FireModuleV2(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, fire_split=2, expand_ratio=4, expand_kernel=5):
        ...
        self.conv = FireModuleV2(
            in_channels, out_channels // fire_split, out_channels, expand_ratio, expand_kernel
        )
...
class FusedMBConv(nn.Sequential):
    def __init__(
        self, in_features: int, out_features: int, expansion: int = 4, explain_kernel: int = 3
    ):
                ...
                residual(
                    nn.Sequential(
                        partial(ConvNormAct, kernel_size=explain_kernel)(
                            in_features, expanded_features, act=nn.ReLU6
                        ),
                        # here you can apply SE
                        # wide -> narrow
                        partial(ConvNormAct, kernel_size=1)(
                            expanded_features, out_features, act=nn.Identity
                        ),
                    ),
                ),
                nn.ReLU(),
...
class DownClose7(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, squeeze_rate=2):
        super().__init__()

        self.encoder = nn.Sequential(
            FusedMBConv(in_channels, out_channels, expansion=4, explain_kernel=3),
            nn.Conv2d(
                out_channels, in_channels // squeeze_rate, kernel_size=3, stride=1, padding=1
            ),
            nn.BatchNorm2d(in_channels // squeeze_rate),
            nn.ReLU(inplace=True),
            nn.Conv2d(
                in_channels // squeeze_rate,
                in_channels // squeeze_rate,
                kernel_size=kernel_size,
                stride=2,
                padding=kernel_size // 2,
            ...
            nn.ReLU(inplace=True),
            nn.Conv2d(
                in_channels // squeeze_rate, out_channels, kernel_size=1, stride=1, padding=1
            ),
            nn.BatchNorm2d(out_channels),
            nn.ReLU(inplace=True),
        )
...
class DownClose6(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, squeeze_rate=2):
        ...
        kernel_conv = None
        if kernel_size == 1:
            kernel_conv = nn.Conv2d(
                in_channels // squeeze_rate, out_channels, kernel_size=1, padding=1
            )
        if kernel_size == 3:
            kernel_conv = nn.Conv2d(
                in_channels // squeeze_rate, out_channels, kernel_size=3, stride=2, padding=1
            )
        if kernel_size == 5:
            kernel_conv = nn.Conv2d(
                in_channels // squeeze_rate, out_channels, kernel_size=5, stride=2, padding=2
            )
        if kernel_size == 7:
            kernel_conv = nn.Conv2d(
                in_channels // squeeze_rate, out_channels, kernel_size=7, stride=2, padding=3
            )
        if kernel_size == 9:
            kernel_conv = nn.Conv2d(
                in_channels // squeeze_rate, out_channels, kernel_size=9, stride=2, padding=4
            )
