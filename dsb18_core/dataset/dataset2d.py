...
class Dataset2D(torch.utils.data.Dataset):
    def __init__(self, df, CFG, subset="train", transforms=None):
        ...
        if self.preprocessing_params is None:
            ...
            self.preprocessing_params = {
                ...
                'resize_mode': getattr(CFG, 'resize_mode', 'resize_only'),  # 'resize_only' or 'pad_and_resize'
                'mask_interpolation': 'linear',
                'intensity_normalization': {
                    'method': 'zscore',  # Z-score normalization for DSB-2018
                    'zscore': {
                        'mean': 0.0,      # Will be computed from fingerprint
                        'std': 1.0
                    }
                }
            }
        ...
        if self.debug_mode:
            ...
            print(f"  [Dataset2D] Debug mode enabled for {subset} subset - saving first 5 samples to {self.debug_dir}")
    ...
    def __getitem__(self, index):
        ...
        if debug_mode and index < 5:
            print(f"[DEBUG] Dataset2D.__getitem__ called: index={index}, subset={self.subset}, debug_mode={debug_mode}", flush=True)
        ...
        if debug_mode and index < 5:  # Only save first 5 samples per subset
            ...
            if img_before.dtype != np.uint8:
                img_before = (img_before * 255).astype(np.uint8) if img_before.max() <= 1.0 else img_before.astype(np.uint8)
        ...
        if self.preprocessing_params is not None:
            img, mask_processed, _ = preprocess_pipeline(
                img, 
                combined_mask if combined_mask.sum() > 0 else None,
                self.preprocessing_params
            )
            if mask_processed is not None:
                combined_mask = mask_processed.squeeze() if len(mask_processed.shape) == 3 else mask_processed
            else:
